...
function googleNativeErrorMessage(error: unknown) {
  ...
  if (lower.includes('network') || lower.includes('7:') || lower.includes('no internet')) {
    return 'Sem conexao com a internet. Tente de novo.'
  }
  if (
    lower.includes('10:') ||
    ...
    return 'Google Sign-In nao esta configurado para este APK (google-services.json ou SHA-1 incorretos). Use e-mail/senha acima para entrar.'
  }
  return 'Google indisponivel: ' + (message || 'erro desconhecido') + '. Use e-mail/senha acima.'
}
...
function RevealStudyTabs({
  ...
  const legacyInlineReveal = false

  if (legacyInlineReveal && active) {
  ...
  const activeLabel = active === 'pronunciation' ? 'Pronuncia' : active === 'literal' ? 'Literal' : ''
  const activeValue = active === 'pronunciation' ? pronunciation : active === 'literal' ? literal : ''
  const activeNote = active === 'literal' ? literalNote : undefined
  ...
  return (
    <div className={compact ? 'reveal-study-tabs compact' : 'reveal-study-tabs'}>
      ...
      {active && (
        <div className="reveal-result-card" role="status" aria-live="polite">
          <div className="reveal-result-main">
            <span className="reveal-result-label">{activeLabel}</span>
            <strong>{activeValue}</strong>
            <button type="button" onClick={() => setActive(null)} aria-label={`Ocultar ${activeLabel.toLowerCase()}`}>
              x
            </button>
          </div>
          {activeNote ? <small>{activeNote}</small> : null}
        </div>
      )}
    </div>
  )
